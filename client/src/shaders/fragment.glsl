...
vec3 calculateIBL(vec3 albedo, vec3 normal, vec3 viewDir, float metallic, float smoothness, vec2 pixelPosInSprite) {
  ...
  // Scale by metallic - higher metallic = stronger IBL intensity
  vec3 iblContribution = (diffuseIBL * 0.5 + specularIBL * 0.7) * uIBLIntensity * metallic;
...
void main(void) {
  ...
  if (uGIEnabled) {
    ...
    // DEBUG: Show LPV texture as overlay (if black, it's empty)
    finalColor = mix(finalColor, lpvData * 10.0, 0.5);
  }
