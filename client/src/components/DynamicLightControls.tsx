...
const DynamicLightControls = ({
  ...
  const [localShadowConfig, setLocalShadowConfig] =
    useState<ShadowConfig>(shadowConfig);
  const [localAOConfig, setLocalAOConfig] = useState<AmbientOcclusionConfig>(
    ambientOcclusionConfig || {
      enabled: false,
      strength: 0.3,
      radius: 25,
  ...
  const [localIBLConfig, setLocalIBLConfig] = useState<{
    enabled: boolean;
    intensity: number;
  ...
  const finishRename = (lightId: string) => {
    ...
    const nameExists = localLights.some(
      (light) => light.id === editingName && light.id !== lightId,
    );
    if (nameExists) {
      setEditingLightId(null);
      setEditingName("");
  ...
  const updateMaskProperty = (
    lightId: string,
    property: keyof MaskConfig,
    value: any,
  ) => {
    const currentMask = localLights.find((l) => l.id === lightId)?.mask;
    if (currentMask) {
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-4 space-y-4 max-h-[80vh] overflow-y-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <div
            ...
            data-testid="dynamic-controls-status"
          ></div>
          <h3
            className="text-sm font-semibold text-card-foreground"
            data-testid="dynamic-controls-title"
          >
            Lighting Controls
          </h3>
        </div>
        ...
        <div className="flex items-center space-x-1">
          <select
            ...
            onChange={(e) =>
              setNewLightType(
                e.target.value as "point" | "directional" | "spotlight",
              )
            }
      ...
      <div className="border-b border-border pb-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          <input
            type="color"
            value={rgbToHex(
              localAmbient.color.r,
              localAmbient.color.g,
              localAmbient.color.b,
            )}
            onChange={(e) => {
      ...
      {(() => {
        const directionalLight = localLights.find(
          ...
          <div className="border-b border-border pb-2">
            ...
            <div className="flex items-center space-x-2 mb-1">
              ...
              <input
                ...
                onChange={(e) =>
                  updateLight(directionalLight.id, {
      ...
      <div className="border-b border-border pb-2">
        <div className="flex items-center space-x-2 mb-2">
          ...
          <button
            onClick={() => {
              const newConfig = {
                ...localShadowConfig,
                enabled: !localShadowConfig.enabled,
              };
        ...
        {localShadowConfig.enabled && (
          <>
            ...
            {/* Removed shadow sharpness and shadow bias sliders - bias is redundant with self-interval skipping */}
              ...
              {localIBLConfig.enabled && (
                <>
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <select
                      value={
                        localIBLConfig.environmentMap?.split("/").pop() ||
                        availableSkyBoxes[0]
                      }
                      onChange={(e) => {
            ...
            {/* Screen Space Reflections (SSR) Controls - 2.5D Reflections */}
            <div className="mt-3 pt-2 border-t border-border/50">
              <div className="flex items-center space-x-2 mb-2">
                <Contrast size={12} className="text-muted-foreground" />
                <h5 className="text-xs font-medium text-muted-foreground">
              ...
              {localSSRConfig.enabled && (
                <>
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    <label className="text-xs text-muted-foreground min-w-[70px]">
                      Max Dist: {localSSRConfig.maxDistance.toFixed(0)}px
                    </label>
                    <input
                      type="range"
                      min="50"
                      ...
                      className="flex-1"
                      data-testid="slider-ssr-max-distance"
                    />
                  </div>

                  <div className="flex items-center space-x-2 mb-1">
                    <label className="text-xs text-muted-foreground min-w-[70px]">
                      Quality: {localSSRConfig.quality.toFixed(0)} steps
                    </label>
                    <input
                      type="range"
                      min="5"
        ...
        .map((light) => {
          ...
          const directionAngle =
            light.type === "directional"
              ? (Math.atan2(light.direction.y, light.direction.x) * 180) /
                  Math.PI +
                180
              : 0;
          ...
          return (
            <div
              key={light.id}
              className="border border-border rounded p-2 bg-muted/5"
            >
              {/* Compact Header */}
                      ...
                      <div className="grid grid-cols-3 gap-1">
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateLight(light.id, {
                                softness: parseFloat(e.target.value),
                              })
                            }
                            className="flex-1"
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateMaskProperty(light.id, "offset", {
                              ...light.mask?.offset,
      ...
      {localLights.length === 0 && (
        <div className="text-center py-6 text-muted-foreground">
          ...
          <p className="text-xs">
            Use the "Add New Light" section above to create lights
          </p>
        </div>
      )}
    </div>
