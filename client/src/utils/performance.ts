...
export interface PerformanceSettings {
  ...
  enableSSR: boolean; // Screen Space Reflections
  textureScale: number;
  fpsTarget: number;
  capFpsTo60: boolean;
...
export const getOptimalSettings = (device: DeviceInfo): PerformanceSettings => {
  if (device.isLowEnd || (device.isMobile && device.memory && device.memory <= 3)) {
    ...
    return {
      ...
      enableSSR: false, // SSR disabled on low-end
      textureScale: 0.5,
      fpsTarget: 30,
      capFpsTo60: true
    };
    ...
    return {
      ...
      enableLightMasks: false,
      enableSSR: false, // SSR disabled on mobile
      textureScale: 0.75,
      fpsTarget: 45,
      capFpsTo60: true
    };
    ...
    if (isHighEnd) {
      return {
        ...
        enableSSR: true, // SSR enabled on high-end only
        textureScale: 1.0,
        fpsTarget: 60,
        capFpsTo60: true
      };
      ...
      return {
        ...
        enableNormalMapping: true,
        enableLightMasks: false,
        enableSSR: false, // SSR disabled on medium
        textureScale: 0.75,
        fpsTarget: 45,
        capFpsTo60: true
      };
